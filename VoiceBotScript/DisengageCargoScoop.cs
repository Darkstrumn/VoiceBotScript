...
//=============================================================================
namespace VoiceBotScriptDisengageCargoScoop
    {
    //=========================================================================
    internal class Program
        ...
        //---------------------------------------------------------------------
        public static void DisengageCargoScoop(string[] args)
            {
            VoiceBotScript.Run(new IntPtr());
            }//</DisengageCargoScoop(string[] args)>
        }/*</class::Program>*/
    ...
    /// Function: Non-Toggle Disengage of Cargo Scoop.
    /// Companion Macro will Engage Cargo Scoop.
    /// <summary>
    //using System;
    //using System.Collections.Generic;//<<--needed for Dictionary
    //using System.Data; //<<--in references
    ...
    /// <summary>
    /// Author: Darkstrumn:\created::160116.00
    /// Function: Non-Toggle Deploy of Cargo Scoop.
    ...
    public static class VoiceBotScript
        ...
        private static string str_script_name = "DisengageCargoScoop";
        ...
        private static string[,] _arr_str_responses = new string[,]
                    {
                    {/*comply verbiage*/
					"Cargo scoop disengaged."
                    ,"Retracting cargo scoop."
                    ,"Sir - cargo scoop is now offline."
                    ,"Acknowledged. cargo scoop inactive and stowed."
                    }
                    ,{/*unale to comply verbiage*/
					"Cargo Scoop is presently closed and disengaged."
                    ,"Commander, cargo scoop is fully stowed."
                    ,"Sir - cargo scoop is already currently offline."
                    ,"Unable to comply; cargo scoop is currently inactive."
                    }
                    };
        ...
        //---------------------------------------------------------------------------
        /// <summary>
        /// Engage Cargo Scoop macro script
        /// Non-toggle type, uses companion macro-script to Disengage Cargo Scoop
        /// </summary>
        /// <param name="windowHandle"></param>
        public static void Run(IntPtr windowHandle)
            {
            string str_cargo_scoop;
            int int_response_index = -1;
            string str_response = "";
            //>>>>>defaults
            str_cargo_scoop = Includes.ShipModel.GetState("cargoScoop");//<<--deployed = 1, 0 = retracted
            str_cargo_scoop = str_cargo_scoop.Length > 0 ? str_cargo_scoop : "0";//<<--handle defaults
            int_response_index = ( new Random().Next(0, _arr_str_responses.GetLength(1)) );
            //
            if ( str_cargo_scoop == "1" )
                {
                str_response = _arr_str_responses[(int)enum_response.ok, int_response_index];
            ...
            else //>>>>>give random "failure" feedback
                {
                str_response = _arr_str_responses[(int)enum_response.fail, int_response_index];
                Includes.FxLib.SaveArgs("AOSCore_ipc", ( "deployCargoScoop|TTS,{TTS}".Replace("{TTS}", str_response) ).Split('|'));//<<--second param is expected to be an array
                }
            //</if(isSet != "1")>
