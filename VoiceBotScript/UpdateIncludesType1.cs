...
//=============================================================================
namespace VoiceBotScriptUpdateIncludesTemplateType1
    {
    //=========================================================================
    internal class Program
        {
        //---------------------------------------------------------------------
        public static void UpdateIncludesTemplateType1(string[] args)
            {
            VoiceBotScript.Run(new IntPtr());
            }//</UpdateIncludesTemplateType1(string[] args)>
        }/*</class::UpdateIncludesTemplateType1>*/
    ...
    //using System.Collections.Generic;//<<--needed for Dictionary
    //using System.Data; //<<--in references
    //using System.Data.Linq;//<<--provided by C:\\Program Files (x86)\\Reference Assemblies\\Microsoft\\Framework\\.NETFramework\\v4.0\\Profile\\Client\\System.Data.Linq.dll
    //using System.Data.OleDb; //<<--provided by System.Data.DataSetExtensions.dll
    //using System.Diagnostics;//<<--needed for debug (visual studio IDE only)
    //using System.Threading;
    //using System.Threading.Tasks;
    ...
    /// Function: Includes\\Library code updater
    /// <summary>
    //=============================================================================
    public static class VoiceBotScript
        ...
        private static string str_script_name = "Update{INCLUDES}Includes".Replace("{INCLUDES}", str_includes_name);
        ...
        private static string _str_using_block = @"using System;
using System.Collections.Generic;//<<--needed for Dictionary
using System.Data; //<<--in references
using System.Linq;//<<--provided by C:\\Program Files (x86)\\Reference Assemblies\\Microsoft\\Framework\\.NETFramework\\v4.0\\Profile\\Client\\System.Data.Linq.dll
using System.Data.OleDb; //<<--provided by System.Data.DataSetExtensions.dll
...
";
        ...
        private static string _str_code_block = @"
//=============================================================================
...
public static class ScriptEngine
  ...
  //-------------------------------------------------------------------------
  /// <summary>
  /// CsCodeAssembler takes provided CS source and attempts to compile code, then returns an instance object of the assembly
  ...
  /// <returns></returns>
  public static object CsCodeAssembler( IntPtr windowHandle, string str_source_cs, string str_references = """")
    {
    object obj_return = null;
    str_references = (str_references.Length != 0 ? str_references : ScriptCore.Includes.SupportClasses.Constants.str_default_references);
    //
  ...
  public static object CallFunction(Object assembly_library_code, string str_function_name, object[] obj_parameters_array)
  	{
    //
    var obj_library_code_instance = ((Assembly)assembly_library_code).CreateInstance(""ScriptCore.Includes."" + str_function_name);
  ...
  public static object GetInclude(IntPtr windowHandle, string str_include,string str_references = """")
    ...
    var obj_include = ScriptEngine.CsCodeAssembler(windowHandle, str_code,str_references);
...
public static class Speech
    ...
    //---------------------------------------------------------------------
    /// <summary>
    /// SpeechRecognizer provide voice prompts, where the user can be prompted
...
public static class FxLib
    {
    //---------------------------------------------------------------------
    public static ScriptCore.Includes.SupportClasses.Args GetArgs(string str_variable_name)
    {
    var args_return = new ScriptCore.Includes.SupportClasses.Args();
    string str_key;
    string str_value;
    BFS.Speech.TextToSpeech(""Diagnostics::BFS.ScriptSettings.ReadValue("" + str_variable_name + "")"" + BFS.ScriptSettings.ReadValue(str_variable_name));
    try {
        foreach ( string element in BFS.ScriptSettings.ReadValue(str_variable_name).Split('|') )
            ...
            args_return.argv.Add(args_return.argc, new ScriptCore.Includes.SupportClasses.Arg() { key = str_key, value = str_value });
            }
        }
    ...
    /// </summary>
    public static void SaveArgs(string str_variable_name, List<string> lst_content)
    {
    string str_content = """";//<<--default to deleting variable value
    string[] arr_str_content = lst_content.ToArray();
    //
    ...
    }/*</SaveArgs(string str_variable_name, List<string> lst_content)>*/
    //-------------------------------------------------------------------------
    }/*</class::FxLib>*/
//=============================================================================
//== /FxLib Function Library
...
namespace SupportClasses
    ...
    public static class Constants
        {
        public static string str_default_references = """ + Includes.SupportClasses.Constants.str_default_references + @""";
        public static string str_default_connection_string = """ + Includes.SupportClasses.Constants.str_default_connection_string + @""";
        //---------------------------------------------------------------------
        }/*</class Constants()>*/
    ...
    public class Args
        ...
        public Args()
            {
            argv = new Dictionary<int, ScriptCore.Includes.SupportClasses.Arg>();//<<--init dictionary
            }//</Args()>
        //-------------------------------------------------------------------------
...
public static class ShipModel
    ...
    public static void ResetShipState()
        {
        ScriptCore.Includes.SupportClasses.Args args_ship_state = FxLib.GetArgs(""ship_state"");
...
,{ ""4.5.x"", ""0""} //<<--subscreen 4 tab 5\\Functions leftRight selection index (reservered for future use)
        ...
        FxLib.SaveArgs(""ship_state"", str_ship_state.Split('|').ToList<string>());
        }//</_ResetShipState()>
    //-------------------------------------------------------------------------
    public static string GetState(string str_property)
        ...
        foreach (KeyValuePair<int, ScriptCore.Includes.SupportClasses.Arg> row in args_ship_state.argv)
            {
	...
	public static void SetState(string str_property, string str_value)
		...
		if(args_ship_state.argc == 0)
			...
			args_ship_state = FxLib.GetArgs(""ship_state"");//<<--reload
			}
		else {; }//>>>>>do nothing
		//</if(if(args_ship_state.argc == 0))>
		ScriptCore.Includes.SupportClasses.Args args_new_ship_state = FxLib.GetArgs(""ship_state"");
		//<diagnostics>BFS.Speech.TextToSpeech(""GetState::Number of data elements to scan is {NUM}."".Replace(""{NUM}"", args_ship_state.argc.ToString()));
		foreach (KeyValuePair<int, ScriptCore.Includes.SupportClasses.Arg> row in args_ship_state.argv)
			{
			int_index++;
		...
		if ( !bln_found )//>>>>>create
			{
			args_ship_state.argv.Add(args_ship_state.argc,new ScriptCore.Includes.SupportClasses.Arg() { key = str_key, value = str_value });
			}
...
public static class DBS
    ...
    //-------------------------------------------------------------------------
    /// <summary>
    /// type extension for true null handling
    /// </summary>
    ...
    public static DataTable Query(string str_query, string str_connectionstring)
        ...
        str_connectionstring = (str_connectionstring.Length > 0 ? str_connectionstring : ScriptCore.Includes.SupportClasses.Constants.str_default_connection_string); // ""\\ScriptExtension\\"" <<--this must be created in voicebot install folder and the .mdb files should be stored in here so the macroscripts have access to a standard location.
        //
        try
            {
...
//=============================================================================
".Replace("{INCLUDES}", str_includes_name).Replace("{REFERENCES}", Includes.SupportClasses.Constants.str_default_references);

        private static string _str_code = @"{USINGBLOCK}
namespace Includes
    {
    //=========================================================================
    {CODEBLOCK}
    //=========================================================================
    }/*</namespace::Includes>*/".Replace("{USINGBLOCK}", _str_using_block).Replace("{CODEBLOCK}", _str_code_block);
